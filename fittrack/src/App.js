  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000dd", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ background: C.card, borderRadius: 20, padding: 32, textAlign: "center", width: 280, border: `1px solid ${C.border}` }}>
        ...
        <div style={{ position: "relative", width: 130, height: 130, margin: "0 auto 20px" }}>
          <svg width="130" height="130" style={{ transform: "rotate(-90deg)" }}>
            ...
            <circle cx="65" cy="65" r={r} fill="none"
              stroke={timeLeft === 0 ? C.accent : timeLeft <= 5 ? C.red : C.accent}
          ...
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <div style={{ fontFamily: "'Bebas Neue'", fontSize: 42, color: timeLeft === 0 ? C.accent : timeLeft <= 5 ? C.red : C.text, lineHeight: 1 }}>
              {timeLeft}
            </div>
        ...
        {timeLeft === 0 && (
          <div style={{ fontFamily: "'Bebas Neue'", fontSize: 18, color: C.accent, marginBottom: 12, letterSpacing: 2 }}>
            🔥 REST COMPLETE!
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => { setTimeLeft(seconds); setRunning(true); }}
            style={{ flex: 1, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px", cursor: "pointer", color: C.muted, fontFamily: "'Outfit'", fontSize: 13 }}>
            🔄 Reset
          </button>
          <button onClick={onClose}
            style={{ flex: 1, background: C.accent, border: "none", borderRadius: 8, padding: "10px", cursor: "pointer", color: "#000", fontFamily: "'Bebas Neue'", fontSize: 15, letterSpacing: 1 }}>
            NEXT SET
          </button>
        </div>
      </div>
...
export default function FitTrack() {
  ...
  const [prs, setPrs] = useState(() => storage.get("ft_prs") || {}); // { "ExName": { weight, reps, date } }
  const [prModal, setPrModal] = useState(null); // { exName, restSecs }
  ...
  const handleGenerate = () => {
    ...
    setCompleted(false);
    setTimeout(() => {
      setWorkout(DEMO_WORKOUTS[selectedMuscle][selectedLevel]);
      setScreen("workout");
      setLoading(false);
    }, 1000);
  };
  ...
  const toggleSet = (exIdx, setIdx, restSecs, exName) => {
    ...
    setSets(prev => {
      const ex = prev[exIdx] || {};
      return { ...prev, [exIdx]: { ...ex, [setIdx]: !ex[setIdx] } };
    });
    if (wasOff) {
      // Show PR modal first, then timer
      setPrModal({ exName, restSecs });
    }
  };
    ...
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Bebas Neue', Impact, sans-serif", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ background: C.card, borderRadius: 16, padding: 24, width: "min(320px, 90vw)", border: `1px solid ${C.accent}44` }}>
            ...
            <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
              <div style={{ flex: 1 }}>
                ...
                <input type="number" value={prWeight} onChange={e => setPrWeight(e.target.value)} placeholder="e.g. 40"
                  style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontFamily: "'Outfit'", fontSize: 16, outline: "none" }} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ fontFamily: "'Outfit'", fontSize: 11, color: C.muted, marginBottom: 6 }}>REPS DONE</div>
                <input type="number" value={prReps} onChange={e => setPrReps(e.target.value)} placeholder="e.g. 10"
                  style={{ width: "100%", background: C.surface, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 12px", color: C.text, fontFamily: "'Outfit'", fontSize: 16, outline: "none" }} />
              </div>
            </div>
            {prs[prModal.exName] && (
              <div style={{ background: C.surface, borderRadius: 8, padding: "8px 12px", marginBottom: 14, fontFamily: "'Outfit'", fontSize: 12, color: C.muted, borderLeft: `3px solid ${C.yellow}` }}>
                🏆 Current PR: <span style={{ color: C.yellow }}>{prs[prModal.exName].weight}kg × {prs[prModal.exName].reps} reps</span>
              </div>
            )}
            <div style={{ display: "flex", gap: 8 }}>
      ...
      {activeVideo && (
        <div style={{ position: "fixed", inset: 0, background: "#000000cc", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => setActiveVideo(null)}>
          <div style={{ background: C.card, borderRadius: 12, overflow: "hidden", width: "min(500px, 94vw)", padding: 24, textAlign: "center" }} onClick={e => e.stopPropagation()}>
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 480, margin: "0 auto", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
        ...
        {/* Nav */}
        <nav style={{ display: "flex", padding: "16px 20px 0", gap: 4 }}>
          {[["home", "🏠", "HOME"], ["prs", "🏆", "PRs"], ["history", "📊", "LOG"], ["settings", "⚙️", "PROFILE"]].map(([s, icon, label]) => (
            <button key={s} onClick={() => setScreen(s === "home" && screen === "workout" ? "workout" : s)}
              style={{ flex: 1, background: (screen === s || (s === "home" && screen === "workout")) ? C.accentDim : "transparent", border: `1px solid ${(screen === s || (s === "home" && screen === "workout")) ? C.accent + "66" : C.border}`, borderRadius: 8, padding: "8px 2px", cursor: "pointer", color: (screen === s || (s === "home" && screen === "workout")) ? C.accent : C.muted, fontSize: 10, letterSpacing: 1, fontFamily: "'Bebas Neue'", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
        ...
        <main style={{ flex: 1, padding: "20px", overflowY: "auto" }}>
              ...
              {workout.map((ex, i) => {
                ...
                return (
                  <div key={i} className="card-hover" style={{ background: C.card, border: `1px solid ${allDone ? C.accent + "44" : C.border}`, borderRadius: 12, padding: 16, marginBottom: 14, opacity: allDone ? 0.85 : 1 }}>
                    ...
                    {/* PR Badge */}
                    {pr && (
                      <div style={{ background: C.accentDim, borderRadius: 6, padding: "4px 10px", marginBottom: 8, fontFamily: "'Outfit'", fontSize: 12, color: C.accent, display: "inline-block" }}>
                    ...
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                      {Array.from({ length: ex.sets }).map((_, s) => {
                        ...
                        return (
                          <button key={s} onClick={() => toggleSet(i, s, ex.rest, ex.name)}
                            style={{ width: 44, height: 44, borderRadius: 8, cursor: "pointer", background: isDone ? C.accent : C.surface, border: `1px solid ${isDone ? C.accent : C.border}`, color: isDone ? "#000" : C.muted, fontSize: 12, fontFamily: "'Outfit'", fontWeight: 600, transition: "all 0.2s" }}>
                            {isDone ? "✓" : `S${s + 1}`}
                          </button>
                        );
                      })}
          ...
          {screen === "prs" && (
            <div className="animate-in">
              ...
              <div style={{ fontFamily: "'Outfit'", fontSize: 13, color: C.muted, marginBottom: 20 }}>
                {Object.keys(prs).length} exercises logged
              </div>
              {Object.keys(prs).length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted, fontFamily: "'Outfit'" }}>
                  <div style={{ fontSize: 48 }}>🏆</div>
                  <div style={{ marginTop: 12, fontSize: 15 }}>Koi PR nahi hai abhi!</div>
                  <div style={{ marginTop: 8, fontSize: 13 }}>Workout karo aur sets complete karte waqt weight log karo</div>
                </div>
              ) : (
          ...
          {screen === "history" && (
            <div className="animate-in">
              ...
              {history.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted, fontFamily: "'Outfit'" }}><div style={{ fontSize: 40 }}>📋</div><div style={{ marginTop: 12 }}>No workouts yet!</div></div>
              ) : history.map((h, i) => (
          ...
          {screen === "settings" && (
            <div className="animate-in">
              ...
              <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: 20, marginBottom: 16 }}>
                ...
                {[["💪","7 Muscle Groups","Chest, Back, Shoulders, Arms, Legs, Core, Full Body"],["⏱️","Rest Timer","Har set ke baad automatic countdown + beep"],["🏆","PR Tracker","Har exercise ka personal record track karo"],["💾","Auto Save","History aur PRs automatically save hoti hain"]].map(([icon,title,desc]) => (
              ...
              <button onClick={() => { if (window.confirm("Saara data delete hoga. Sure?")) { localStorage.clear(); setHistory([]); setStreak(0); setMemberName(""); setNameInput(""); setPrs({}); showToast("Data cleared!", "error"); } }} style={{ width: "100%", background: "transparent", border: `1px solid ${C.red}44`, borderRadius: 10, padding: 14, cursor: "pointer", color: C.red, fontFamily: "'Outfit'", fontSize: 14 }}>🗑️ Reset All Data</button>
            </div>
          )}
        </main>
      </div>
